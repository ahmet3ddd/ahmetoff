...
(function (global) {
    ...
    function exportSTL(result, extrudeDepth, filename) {
        filename = filename || 'kemer.stl';
        const geometries = [];

        for (const unit of result.archUnits) {
            if (!unit.outer) continue;
        ...
        if (geometries.length === 0) {
            throw new Error('STL icin kapali profil yok (Kalinlik > 0 gerekli).');
        }
