...
(function () {
    ...
    const state = {
        ...
        fillet78: DEFAULTS.fillet78,
        autoFilletSize: DEFAULTS.autoFilletSize,
        filletSize: DEFAULTS.filletSize,
        filletSegments: DEFAULTS.filletSegments,
    ...
    const el = {
        ...
        fillet78: document.getElementById('param-fillet-on'),
        filletSize: document.getElementById('param-fillet-size'),
        autoFilletSize: document.getElementById('param-auto-fillet-size'),
        filletSegments: document.getElementById('param-fillet-seg'),
        panelFillet: document.getElementById('panel-fillet'),
        readoutFillet: document.getElementById('readout-fillet'),
    ...
    function snapshotState() {
        return {
            ...
            fillet78: state.fillet78,
            autoFilletSize: state.autoFilletSize,
            filletSize: state.filletSize,
            filletSegments: state.filletSegments,
    ...
    function syncUIFromState() {
        ...
        el.fillet78.checked = state.fillet78;
        el.autoFilletSize.checked = state.autoFilletSize;
        el.filletSize.value = state.filletSize;
        el.filletSegments.value = state.filletSegments;
    ...
    function applySnapshot(snap) {
        ...
        state.fillet78 = snap.fillet78 != null ? snap.fillet78 : false;
        state.autoFilletSize = snap.autoFilletSize != null ? snap.autoFilletSize : true;
        state.filletSize = snap.filletSize != null ? snap.filletSize : 0;
        state.filletSegments = snap.filletSegments != null ? snap.filletSegments : 3;
    ...
    function syncAra78PanelVisibility() {
        ...
        if (el.panelFillet) {
            el.panelFillet.hidden = (state.type !== 'badem');
        }
    }

    function syncFilletInputs() {
        el.filletSize.disabled = !state.fillet78 || state.autoFilletSize;
        el.autoFilletSize.disabled = !state.fillet78;
        el.filletSegments.disabled = !state.fillet78;
        if (state.type !== 'badem' || !state.autoFilletSize) return;
        ...
        cut.autoAraMesafe78 = state.autoAraMesafe78;
        cut.araMesafe78 = state.araMesafe78;
        const auto = Geom.calcFillet78Auto(p1, p2, p3, state.asaba, cut);
        state.filletSize = Math.round(auto * 100) / 100;
        el.filletSize.value = state.filletSize;
    }
    ...
    /**
     * Fillet'i o anki (duzenlenmis) geometriye uygula. Duzenleme modunda
     * yukseklikler degistikce yay her seferinde guncel yuzey acilarina
     * gore yeniden hesaplanir.
     */
    function applyFilletToResult(result) {
        if (state.type !== 'badem' || !state.fillet78) return result;
        return Geom.applyFillet78(result, {
            filletSegments: state.filletSegments,
            filletSize: state.filletSize,
            autoFilletSize: state.autoFilletSize
        });
    ...
    function refreshModifyView() {
        const display = applyFilletToResult(
            Modify.applyToResult(generateBaseResult(), state.modifyState)
        );
        ...
        el.readoutVerts.textContent = display.mesh
            ...
            : '\u2014';
        if (el.readoutFillet) {
            el.readoutFillet.textContent = display.filletInfo
                ? display.filletInfo.size.toFixed(2) + ' birim / ' +
                  display.filletInfo.segments + ' segment'
                : '\u2014';
        }
    ...
    function getViewer3DOptions(result) {
        ...
        if (state.showVertexNumbers3d) {
            ...
            opts.p3 = p3;
            // Fillet yay koselerini numaralandirma; orijinal numaralama sabit kalsin
            opts.vertexLabelLimit = result.filletInfo ? result.filletInfo.firstExtraIndex : 0;
        }
    ...
    function refresh(fitCam) {
        ...
        // Fillet en son, guncel geometri uzerinde hesaplanir
        result = applyFilletToResult(result);
        ...
        el.readoutVerts.textContent = result.mesh
            ...
            : '\u2014';
        if (el.readoutFillet) {
            el.readoutFillet.textContent = result.filletInfo
                ? result.filletInfo.size.toFixed(2) + ' birim / ' +
                  result.filletInfo.segments + ' segment'
                : '\u2014';
        }
    ...
    function bindParam(input, key, parser, keepModify) {
        ...
        function applyValue() {
            ...
            if (Number.isFinite(v)) {
                state[key] = v;
                // Fillet parametreleri duzenleme durumunu etkilemez; fillet
                // sonradan guncel geometriye uygulanir
                if (state.mode === 'modify' && !keepModify) {
                    state.modifyState = null;
                }
                refresh(false);
            }
    ...
    bindParam(el.filletSize, 'filletSize', null, true);
    bindParam(el.filletSegments, 'filletSegments', function (v) {
    ...
    el.fillet78.addEventListener('change', function () {
        pushHistory();
        state.fillet78 = el.fillet78.checked;
        refresh(false);
    });

    el.autoFilletSize.addEventListener('change', function () {
        pushHistory();
        state.autoFilletSize = el.autoFilletSize.checked;
        refresh(false);
    });
    ...
    el.btnReset.addEventListener('click', function () {
        ...
        state.fillet78 = DEFAULTS.fillet78;
        state.autoFilletSize = DEFAULTS.autoFilletSize;
        state.filletSize = DEFAULTS.filletSize;
        state.filletSegments = DEFAULTS.filletSegments;
        ...
        el.fillet78.checked = DEFAULTS.fillet78;
        el.autoFilletSize.checked = DEFAULTS.autoFilletSize;
        el.filletSize.value = DEFAULTS.filletSize;
        el.filletSegments.value = DEFAULTS.filletSegments;
