...
(function (global) {
    ...
    class Viewer3D {
        ...
        _drawVertexNumbers(vertices, p3, limit) {
            if (!vertices || vertices.length === 0) return;

            // limit > 0: fillet yay koseleri gibi eklenen koseleri etiketleme
            const labelCount = (limit && limit > 0)
                ? Math.min(limit, vertices.length)
                : vertices.length;
        ...
        update(result, viewOptions) {
            ...
            if (viewOptions.showVertexNumbers && result.mesh.vertices) {
                this._drawVertexNumbers(
                    result.mesh.vertices,
                    viewOptions.p3,
                    viewOptions.vertexLabelLimit
                );
            }
        }
