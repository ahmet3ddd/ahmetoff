...
(function (global) {
    ...
    /** Ardisik numaralari "v19-v28" araligina sikistir (fillet listeleri uzun) */
    function formatRunList(nums) {
        ...
        while (i < sorted.length) {
            ...
            if (j - i >= 2) {
                parts.push('v' + sorted[i] + '\u2013v' + sorted[j]);
            } else {
                for (let k = i; k <= j; k++) parts.push('v' + sorted[k]);
            }
            i = j + 1;
        }
        return parts.join(', ');
    ...
    function formatMoveMap(profile) {
        ...
        for (let i = 0; i < SEG_LABELS.length; i++) {
            ...
            if (nums && nums.length) {
                lines.push(label + ' \u2192 ' + formatRunList(nums));
            }
        }
        return lines.join('\n');
    }
