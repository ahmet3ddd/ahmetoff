...
(function (global) {
    ...
    function loadImage(src) {
        return new Promise(function (resolve, reject) {
            ...
            img.onerror = function (e) { reject(new Error('SVG yüklenemedi: ' + e)); };
            img.src = src;
        });
    }
    ...
    function canvasToDownload(canvas, filename) {
        return new Promise(function (resolve, reject) {
            canvas.toBlob(function (blob) {
                if (!blob) {
                    reject(new Error('Canvas to Blob başarısız'));
                    return;
                }
    ...
    const exportPNG = {
        from2D: function (viewer2d, filename) {
            ...
            if (!svg) return Promise.reject(new Error('2D SVG bulunamadı'));
        ...
        from3D: function (viewer3d, filename) {
            ...
            if (!canvas) return Promise.reject(new Error('3D canvas bulunamadı'));
            return canvasToDownload(canvas, filename);
        }
    };
